...
import hashlib
import requests
utils = importlib.import_module('VITS-fast-fine-tuning.utils')
...
def get_speakers():
    try:
        ...
        if not folder or not model_dir:
            return jsonify({"code": 400, "msg": "缺少 folder 或 model_dir"}), 400

        username = request.user['username']
        config_path = USER_DATA_ROOT / username / folder / "model" / model_dir / "config.json"
        if not config_path.is_file():
            return jsonify({"code": 404, "msg": "config.json 不存在"}), 404

        # 读取 config
        hps = utils.get_hparams_from_file(str(config_path))

        # 关键：只读 model.speakers
        speakers_dict = getattr(hps.model, "speakers", {})
        speakers = list(speakers_dict.values())  # ["A", "B", "C"]

        return jsonify({"speakers": speakers}), 200
    except Exception as e:
        logger.exception("get_speakers error")
        return jsonify({"code": 500, "msg": str(e)}), 500
...
def synthesize_speech():
    """
    语音合成接口
    - speaker: 说话人名称（如 "A", "B", "C"）
    - 自动从 config.json 的 model.speakers 映射为 ID
    """
    try:
        # ---------- 1. 解析请求 ----------
        ...
        folder       = data.get('folder')
        model_dir    = data.get('model_dir')
        model_pth    = data.get('model_pth')
        language     = data.get('language')
        text         = data.get('text')
        speaker_name = data.get('speaker')         # 名称，如 "A"
        ...
        # ---------- 2. 必填校验 ----------
        required = [folder, model_dir, model_pth, language, text, speaker_name]
        if not all(required):
        ...
        # ---------- 6. 读取 config 并提取 model.speakers ----------
        try:
            hps = utils.get_hparams_from_file(str(config_json))
        except Exception as e:
            logger.error(f"读取 config.json 失败: {e}")
            return jsonify({"code": 500, "msg": "config.json 解析失败"}), 500

        if not hasattr(hps, 'model') or not hasattr(hps.model, 'speakers'):
            return jsonify({"code": 400, "msg": "config.json 中缺少 model.speakers"}), 400

        speakers_dict = hps.model.speakers  # {"0": "A", "1": "B"}
        name_to_id = {v: k for k, v in speakers_dict.items()}  # {'A': '0', 'B': '1'}

        if speaker_name not in name_to_id:
            available = list(name_to_id.keys())
            return jsonify({
                "code": 400,
                "msg": f"说话人 '{speaker_name}' 不存在，可用: {available}"
            }), 400

        speaker_id = name_to_id[speaker_name]  # "A" → "0"

        # ---------- 7. 定位推理脚本 ----------
        inference_script = Path(VITS_INFERENCE_DIR) / f"{language}_inference.py"
        if not inference_script.is_file():
            return jsonify({"code": 404, "msg": f"语言脚本不存在：{inference_script}"}), 404
        ...
        cmd = [
            ...
            "-c", str(config_json),
            "-t", text,
            "-s", speaker_id,           # 使用映射后的 ID
            "-l", language,
            "-ls", str(length_scale),
            "--noise_scale", str(noise_scale),
            "--noise_scale_w", str(noise_scale_w),
        ...
        if proc.returncode == 0:
            ...
            logger.info(f"合成成功 | 用户: {username} | 说话人: {speaker_name}({speaker_id}) | 文件: {output_path}")
            return jsonify({
                "message": "语音合成成功",
                "audio_url": audio_url
            }), 200
        else:
            logger.error(f"合成失败 | stderr: {proc.stderr}")
            return jsonify({
                "code": 400,
                "msg": "合成失败",
